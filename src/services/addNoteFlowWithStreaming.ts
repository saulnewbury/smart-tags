// services/addNoteWithStreaming.ts

import type { StoreState, Topic, NoteSummary } from '../types'
...
import { streamSummaryFromAPI } from '../utils/streaming'
...
export interface StreamingCallbacks {
  ...
  onStreamPartial?: (partial: any) => void
...
export async function addNoteFlowWithStreaming(
  params: CreateNoteParams,
  store: StoreState,
  streamingCallbacks?: StreamingCallbacks
) {
  console.log('addNoteFlowWithStreaming received params:', {
  ...
  // 1) Stream the summary and get all fields
  let summaryData: any

  try {
    summaryData = await streamSummaryFromAPI(transcript, userPrompt, {
      onToken: (token) => {
        streamingCallbacks?.onStreamToken?.(token)
      },
      onPartialJSON: (partial) => {
        streamingCallbacks?.onStreamPartial?.(partial)
      },
      onComplete: (data) => {
        console.log('Streaming complete, received:', data)
        streamingCallbacks?.onStreamComplete?.()
      },
      onError: (error) => {
        console.error('Streaming error:', error)
        streamingCallbacks?.onStreamError?.(error)
      }
    })
  } catch (error) {
    streamingCallbacks?.onStreamError?.(error as Error)
    throw error
  }

  const { summary, canonical_name, keywords, subjects } = summaryData
